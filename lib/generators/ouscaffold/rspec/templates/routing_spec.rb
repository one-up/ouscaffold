...
describe <%= controller_class_name %>Controller do
  describe "routing" do
    ...
    it "recognizes and generates #confirm_new" do
      { :post => "/<%= table_name %>/confirm_new" }.should route_to(:controller => "<%= table_name %>", :action => "confirm_new")
    end

    it "recognizes and generates #confirm_edit" do
      { :put => "/<%= table_name %>/1/confirm_edit" }.should route_to(:controller => "<%= table_name %>", :action => "confirm_edit", :id => "1")
    end
    <%- end -%>
